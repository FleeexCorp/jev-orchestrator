...
- `decide worker`: Jev picks the worker and model per subtask from a catalog (Codex models from the local Codex cache, Claude subagent aliases, config entries); `workers list`; `codex run --reasoning`.
